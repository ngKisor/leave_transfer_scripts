...
# LEAVE_TYPE_ID 9 for Sick leave 
# Transfer available Sick Leave from past fiscal year to current fiscal year for employee other then permanent.
...
def get_employees_list_for_credit_leaves():
    ...
    WHERE lc.fiscal_year_id=(SELECT fy.id FROM fiscal_year fy WHERE fy.is_current=0 AND fy.is_transferred=0 ORDER BY fy.date_created DESC LIMIT 1)
        AND lc.leave_type_id=9
        AND u.emp_status <> "Permanent"
...
def map_final_data(x, fiscal_year_id):
    ...
    if 'total_credit_leave_days' in x:
...
def prepare_final_list(l1, l2, fiscal_year_id):
    ...
    return list(filter(lambda i: i, map(lambda x: map_final_data(x, fiscal_year_id), d.values())))
...
def main():
    ...
    row_count = add_new_rows_in_leave_credits_for_current_fiscall_year(
        final_data)

    print(f"{row_count} rows inserted in leave_credits")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print('***Execution stopped with ERROR***\n',
